...
def valid_number?(num)
  num_in_cents = (num.to_f * 100).round # no need to call to_i because round will return a Fixnum
  num_in_cents > 0
end
...
loop do
  ...
  fixed_monthly_payment = loan_amount * 
                          (apr_in_months * (1 + apr_in_months)**loan_duration) / 
                          (((1 + apr_in_months)**loan_duration) - 1)

  prompt("Your fixed monthly payment is #{format('%02.2f', fixed_monthly_payment)} USD.")
  sleep 1

  prompt("Would you like to make another calculation? (Y to continue / N to exit / Exits for any other input)")

  ans = gets.chomp

  if ans.downcase == 'n' || ans.downcase != 'y'
    prompt("Bye!")
    break
  else
    prompt("OK.. Resetting...")
